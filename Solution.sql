...
SELECT Distinct Category, Count(*) as No_of_bikes
FROM Bike
GROUP BY 1
Having Count(*)>2
...
SELECT c.name, COUNT(m.customer_id) as Total_memberships
FROM Customer c
...
SELECT Extract(Year FROM m.Start_date) as Year, Extract(Month FROM m.Start_date) as Month, mt.name as Membership, sum(m.total_paid) as Total_revenue
FROM membership m
...
SELECT mt.name as Membership_type, extract (Month FROM m.Start_date) as Month, sum(m.Total_paid) as Total_revenue
FROM membership m
JOIN Membership_type mt
ON mt.id = m.membership_type_id
WHERE Extract(year FROM m.start_date) = 2023
GROUP BY CUBE(membership_type, month)
Order by membership_type, month
